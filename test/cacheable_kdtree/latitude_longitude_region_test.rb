...
class CacheableKdtree::LatitudeLongitudeRegionTest < Minitest::Test
  describe 'initialize' do
    it 'should find the min and max values for each side of the region when given a pair of coordinates' do
      result = CacheableKdtree::LatitudeLongitudeRegion.new(1, 2, 3, 0)
  ...
  describe 'merge_point' do
    it 'should find the min and max values for each side of the region when merging in a new coordinate pair' do
      region = CacheableKdtree::LatitudeLongitudeRegion.new(1, 2, 3, 0)
  ...
  describe 'merge_region' do
    it 'should find the min and max values for each region' do
      region1 = CacheableKdtree::LatitudeLongitudeRegion.new(1, 2, 3, 0)
      region2 = CacheableKdtree::LatitudeLongitudeRegion.new(5, 1, -1, 1)
  ...
  describe 'point_in_region?' do
    before do
      @class_under_test = CacheableKdtree::LatitudeLongitudeRegion.new(1, 4, 4, 0)
    end
  ...
  describe 'regions_intersect?' do
    it 'should return false if the min latitude coordinate is greater than the max of the other' do
      r1 = CacheableKdtree::LatitudeLongitudeRegion.new(2, 3, 4, 4)
      r2 = CacheableKdtree::LatitudeLongitudeRegion.new(0, 3, 1, 4)

      refute CacheableKdtree::LatitudeLongitudeRegion.regions_intersect?(r1, r2)
    end

    it 'should return false if the max latitude coordinate is less than the min of the other' do
      r1 = CacheableKdtree::LatitudeLongitudeRegion.new(-2, 3, 0, 4)
      r2 = CacheableKdtree::LatitudeLongitudeRegion.new(1, 3, 2, 4)

      refute CacheableKdtree::LatitudeLongitudeRegion.regions_intersect?(r1, r2)
    end

    it 'should return false if the min longitude coordinate is greater than the max of the other' do
      r1 = CacheableKdtree::LatitudeLongitudeRegion.new(1, 5, 2, 6)
      r2 = CacheableKdtree::LatitudeLongitudeRegion.new(1, 3, 2, 4)
    ...
    it 'should return false if the max longitude coordinate is less than the min of the other' do
      r1 = CacheableKdtree::LatitudeLongitudeRegion.new(1, 0, 2, 1)
      r2 = CacheableKdtree::LatitudeLongitudeRegion.new(1, 3, 2, 4)
    ...
    it 'should return true if the regions intersect' do
      r1 = CacheableKdtree::LatitudeLongitudeRegion.new(2, 2, 6, 6)
      r2 = CacheableKdtree::LatitudeLongitudeRegion.new(0, 0, 4, 4)

      assert CacheableKdtree::LatitudeLongitudeRegion.regions_intersect?(r1, r2)
    end
  end
end
